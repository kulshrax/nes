...
impl Instruction {
    ...
    pub(super) fn fetch(memory: &dyn Bus, pc: &mut Address) -> Result<Self> {
        use Instruction::*;
...
fn read_addr(memory: &dyn Bus, pc: &mut Address) -> Address {
...
fn read_byte(memory: &dyn Bus, pc: &mut Address) -> u8 {
